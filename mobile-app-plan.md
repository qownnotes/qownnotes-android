...
- GPL-3.0-only licensing, privacy documentation, localized F-Droid metadata, and unsigned
  release-build verification before stable GitHub publication. Official F-Droid publishing remains
  controlled by its independent build infrastructure and tag-based update checks.
- Deterministic F-Droid and README screenshots generated by a focused instrumentation test with
  fixed fake account and note data, plus release-time APK signature-copy verification for
  reproducible F-Droid publication.
